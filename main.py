...
def on_click(x, y):
	...
	drone['force'] += force
	print("Applying force => {}, {}  {},{}".format(force[0], force[1], drone['force'][0], drone['force'][1] ))
...
def update():
	...
	propportional_force = get_proportional_error(display['center'], drone['pos'])

	# (I) Integral force
	integral_force = np.array([0, 0], dtype='float64')
	for entry in drone['pos_history']:
		integral_force -= np.array(get_proportional_error(entry, display['center']), dtype='float64')

	# (D) Derivative force
	derivative_force = np.array([0, 0])

	if len(drone['pos_history']) > 1:
		prev_error = get_proportional_error(drone['pos_history'][-1], display['center'])
		prev_prev_error = get_proportional_error(drone['pos_history'][-2], display['center'])
		derivative_force =  prev_prev_error - prev_error

	# Apply the forces
	drone['force'] += propportional_force * drone['p'] + integral_force * drone['i'] + derivative_force * drone['d'] 
...
def draw_drone(x, y):
	gameDisplay.blit(drone['image'], (x - int(drone['width'] / 2), y - int(drone['height'] / 2)))

def draw():
	# gameDisplay.blit(display['background'], [0, 0])
	draw_drone(drone['pos'][0], drone['pos'][1])
	pygame.draw.line(gameDisplay, (255, 0, 0), (drone['pos'][0], drone['pos'][1]), (applied_force[0], applied_force[1]))
	pygame.draw.circle(gameDisplay, (0, 0, 0), (int(display['center'][0]), int(display['center'][1])), 10, 5)
